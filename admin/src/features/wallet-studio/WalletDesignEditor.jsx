      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <fieldset disabled={!branded} className="flex flex-col gap-5 disabled:opacity-60">
          <div className="flex flex-col gap-4">
            ...
            {/* Photo behind the stamps. The one image covers both platforms — it
                is cover-cropped to the Apple strip and again to the (narrower)
                Google hero, so it is never letterboxed or stretched. Turning the
                stamps off leaves the artwork alone as a plain photo band. */}
            {editable('strip_bg_image_url') && (
              <div>
                <FileUpload
                  label={t('walletDesign.stripArtwork')}
                  onUploaded={set('strip_bg_image_url')}
                  initial={design.strip_bg_image_url || null}
                />
                <p className="mt-1 text-xs text-tx-3">{t('walletDesign.stripArtworkHint')}</p>
                {design.strip_bg_image_url && (
                  <div className="mt-2 flex items-center gap-3">
                    <Toggle
                      checked={design.strip_stamps_visible !== false}
                      onChange={set('strip_stamps_visible')}
                      label={t('walletDesign.stripStampsVisible')}
                    />
                    <button
                      type="button"
                      onClick={() => set('strip_bg_image_url')('')}
                      className="text-xs text-tx-3 underline hover:text-tx-2"
                    >
                      {t('walletDesign.stripArtworkRemove')}
                    </button>
                  </div>
                )}
              </div>
