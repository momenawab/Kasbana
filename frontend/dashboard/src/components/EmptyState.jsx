...
export default function EmptyState({ icon: Icon, title, body, action }) {
  return (
    <div className="flex flex-col items-center justify-center gap-2 rounded-card border border-dashed border-line bg-surface p-10 text-center">
      {Icon && <Icon size={32} className="text-tx-3" />}
      <h3 className="font-head text-lg font-semibold text-tx">{title}</h3>
